...
<style>
  :root{{
    ...
    --line:#ecedf4;
    --shadow:0 18px 40px rgba(43,36,71,.12), 0 2px 6px rgba(43,36,71,.06);
    --shadow-soft:0 10px 24px rgba(43,36,71,.10);
    --r:22px; --r-sm:14px;
  ...
  *{{box-sizing:border-box}} html,body{{margin:0}}
  ...
  .col-body{{display:flex;flex-direction:column;gap:11px;max-height:800px;overflow:auto;padding:2px}}
...
document.getElementById("kpis").innerHTML=[
  ...
  {{n:nDone,lbl:"已完成件數",sub:"",cls:"",ic:"i-mint",svg:'<circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" stroke-width="2"/><path d="M9 12l2 2 4-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>'}},
  ...
  {{n:overdueList.length,lbl:"落後任務件數",sub:"",cls:overdueList.length?"alert":"",ic:"i-red",svg:'<circle cx="12" cy="12" r="8" fill="none" stroke="currentColor" stroke-width="2"/><path d="M12 8v4l3 2" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>'}}
].map(k=>`<div class="kpi ${{k.cls}}"><div class="ico ${{k.ic}}"><svg viewBox="0 0 24 24">${{k.svg}}</svg></div><div class="num">${{k.n}}</div><div class="lbl">${{k.lbl}}</div></div>`).join("");
...
(function(){{
  ...
  segs.forEach(s=>{{if(!s.v)return;const len=s.v/total*C;
    svg+=`<circle cx="${{cx}}" cy="${{cy}}" r="${{r}}" fill="none" stroke="${{STC[s.k]}}" stroke-width="20" stroke-dasharray="${{len}} ${{C-len}}" stroke-dashoffset="${{-off}}" transform="rotate(-90 ${{cx}} ${{cy}})" stroke-linecap="butt"/>`;
    off+=len;}});
...
(function(){{
  ...
  const bars=projects.map(p=>{{
    ...
    return `<div class="bar-row"><div class="top"><span class="name">${{esc(p.name)}}</span><span class="val">${{p.n}} 筆</span></div>
      <div class="track bare"><div class="bar-tip-wrap" style="width:${{barW}}%;height:13px;display:inline-block;vertical-align:top">
        <span style="display:block;width:100%;height:100%;border-radius:999px;background:${{grad}};cursor:pointer"></span>
        <div class="bar-tip">${{tip}}</div></div></div></div>`;
  }}).join("");
...
(function(){{
  ...
  function ownerTable(tasks){{
    ...
    ${{tasks.map(t=>`<tr><td><span class="tg proj">${{esc(t.proj)}}</span></td>
      ...
      <td><span class="mini"><i style="width:${{t.progress}}%"></i></span> <span style="font-size:12px;font-weight:700">${{t.progress}}%</span></td>
      <td>${{t.end?esc(t.end):'<span style="color:var(--ink3)">—</span>'}}${{isOverdue(t)?` <span class="late" style="font-size:11px">逾${{daysLate(t.end)}}天</span>`:''}}</td>
      <td><span class="tg ${{prioCls(t.prio)}}">${{t.prio}}</span></td></tr>`).join("")}}</tbody></table>`;
  }}
